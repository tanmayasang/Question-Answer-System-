...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    elseif (!empty($keyword) &&  $keyword!='Enter a search keyword' && !empty($_POST["topic"])){
        ...
        $sql = "with temp as 
        (SELECT q_id, title, q_body, a_timestamp FROM questions natural join answers
        WHERE MATCH (q_body)
        ...
        select q_id, title, q_body, max(a_timestamp)  from temp group by q_id order by a_timestamp DESC" ;
